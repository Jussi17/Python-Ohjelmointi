...
class Osake:
    def __init__(self, nimi: str, ostohinta: float, maara: int):
        if ostohinta <= 0 or maara <= 0:
            raise ValueError("Ostohinnan ja määrän on oltava suurempia kuin nolla.")
    ...
    @staticmethod
    def laske_tuotto_yhdelle_vuodelle(kasvuprosentti: float, hinta: float):
        return hinta * (1 + kasvuprosentti / 100)

    def tulosta_arvo(self, kasvuprosentti: float, vuodet: int) -> float:
        nykyinen_arvo = self.ostohinta
        for _ in range(vuodet):
            nykyinen_arvo = self.laske_tuotto_yhdelle_vuodelle(kasvuprosentti, nykyinen_arvo)

        osakepotin_arvo = nykyinen_arvo * self.maara
        tuotto = osakepotin_arvo - (self.ostohinta * self.maara)

        print(f"{self.nimi} {self.maara} {self.ostohinta:.2f}")
        print(f"Osakkeen potin arvo on {osakepotin_arvo:.2f} ja tuotto {tuotto:.2f}\n")

        return osakepotin_arvo
...
if __name__ == "__main__":
    ...
    while True:
        ...
        jatka = input("Lisätäänkö uusi osake (k/e)? ").strip().lower()
        if jatka != 'k':
            break
    ...
    koko_potin_arvo = sum(osake.tulosta_arvo(kasvuprosentti, vuodet) for osake in osakkeet)
